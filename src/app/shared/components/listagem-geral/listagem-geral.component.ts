...
export class ListagemGeralComponent {
  ...
  constructor(private obraDeArteService: ObraDeArteService,  private mensagemService: MensagemService) {
  ...
  darLance(obraId: string) {
    ...
    if (valorLance && obra) {
      ...
      if (valorLanceNumber > (obra.valorInicial || obra.valorInicial)) {
        ...
        this.mensagemService.MensagemSucesso(`Lance de R$ ${valorLanceNumber} dado com sucesso!`);
        console.log('Lance dado com sucesso:', valorLanceNumber);
      } else {
        this.mensagemService.MensagemErro('O lance deve ser maior que o valor atual.');
      }
    } else {
